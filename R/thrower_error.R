...
#' @keywords internal
errorData <- function(data, id.col, time.col, eligible.col, treatment.col, outcome.col, causal_contrast){
  ...
  if(missing(outcome.col)) stop("Outcome column name is missing")  else if(!outcome.col %in% names(data)) stop(sprintf("The 'outcome' argument '%s' is not found in the data column names"))
  ...
  if(missing(eligible.col)) stop("Treatment column name is missing")  else if(!treatment.col %in% names(data)) stop(sprintf("The 'treatment' argument '%s' is not found in the data column names"))
  if(missing(method)) stop("'causal_contrast' must be specified")
  }
