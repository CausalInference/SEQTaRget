...
#' @keywords internal
errorData <- function(data, id, time, treatment, elligible){
  ...
  if(missing(elligible)) stop("Elligible column name is missing")  else if(!treatment %in% names(data)) stop(sprintf("The 'elligible' argument '%s' is not found in the data column names"))
}
