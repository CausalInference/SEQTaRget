...
| `weight.stabilized` | Use stabilized IP weights; `FALSE` fits no numerator models and uses unstabilized weights (cumulative products of `1/denominator`) for the treatment weights only | Logical | `TRUE` |
...
| `weight.spline.df` | Degrees of freedom passed to `splines::ns()` when `weight.spline = TRUE` | Integer | `4L` |
