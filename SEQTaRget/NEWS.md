...
* Add `weight.stabilized` to `SEQopts()` (default `TRUE`, the previous behaviour). `weight.stabilized = FALSE` estimates unstabilized IP weights for the treatment weight models: no numerator models are fit and the weights are cumulative products of `1/denominator`, with any supplied `numerator` ignored with a warning. The LTFU (`cense`) and `visit` weight models keep their numerators. Unstabilized weights are typically far more variable, so consider `weight.lower`/`weight.upper` or `weight.p99` truncation.
