...
* Fix subgroup bootstrap results being matched to subgroups by position rather than by name. A bootstrap resample that happened to draw no one from a small subgroup fit no model for it, so for that resample every later subgroup picked up its neighbour's model, and the last subgroup failed with `subscript out of bounds`. Results are now matched by name; resamples with no one from a subgroup are left out of that subgroup's confidence intervals, with a warning giving how many, and a subgroup missing from every resample is reported without a confidence interval. The same applies to the hazard ratio and end-of-follow-up estimates.
* Fix the excused-censoring weights (`excused = TRUE` or `deviation.excused = TRUE`), which depended on the order of `treat.level` in the same way as the non-excused treatment weights, and were wrong in the default settings for post-expansion weights. With `weight.preexpansion = FALSE` the numerator and denominator models predict the probability of being censored, but only the first arm in `treat.level` had it converted to the probability of remaining uncensored: in `SEQdata` with the default `treat.level = c(0, 1)`, arm-1 stayers had a numerator of 0.010 and a denominator of 0.013 where both should be about 0.99, and cumulative weights reached around 1e16, so the outcome model failed to fit unless the weights were truncated (e.g. `weight.upper = 1`). Every uncensored row now gets the probability of remaining uncensored. With `weight.preexpansion = TRUE` the denominator is a treatment model and is now flipped by the treatment received, as for the non-excused weights: binary analyses with the default `treat.level = c(0, 1)` are unchanged, while `treat.level = c(1, 0)` previously inverted both arms and failed to fit, and `multinomial = TRUE` inverted the first arm.
