...
internal.weights <- function(DT, data, params) {
  ...
  if (!params@pre.expansion) {
    ...
    baseline.lag <- data[, subtable.kept, with = FALSE][, tx_lag := shift(get(params@treatment)), by = eval(params@id)][data[, .I[1L], by = eval(params@id)]$V1, tx_lag := 0][, eval(params@treatment) := NULL]

    setnames(baseline.lag, 2, params@time)
    # conditional join
    weight <- rbind(
      DT[followup == 0, ][baseline.lag, on = c(params@id, params@time), nomatch = 0],
      DT[, tx_lag := shift(get(params@treatment)), by = c(eval(params@id), "trial")][followup != 0, ]
    )[, paste0(params@time, params@squared.indicator) := get(params@time)^2][, isExcused := cumsum(ifelse(is.na(isExcused), 0, isExcused)), by = c(eval(params@id), "trial")]
  } else {
    weight <- data[, tx_lag := shift(get(params@treatment)), by = eval(params@id)][get(params@time) == 0, tx_lag := 0][, paste0(params@time, "_sq") := get(params@time)^2]
  }
  ...
  if (!params@excused) {
    ...
    out <- weight[tx_lag == 0, `:=`(
      numerator = predict(numerator0, newdata = .SD, type = "response"),
      denominator = predict(denominator0, newdata = .SD, type = "response")
    )][tx_lag == 0 & get(params@treatment) == 0, `:=`(
      numerator = 1 - numerator,
      denominator = 1 - denominator
    )][tx_lag == 1, `:=`(
      numerator = predict(numerator1, newdata = .SD, type = "response"),
      denominator = predict(denominator1, newdata = .SD, type = "response")
    )][tx_lag == 1 & get(params@treatment) == 0, `:=`(
      numerator = 1 - numerator,
      denominator = 1 - denominator
    )]
    ...
    out <- weight[tx_lag == 0 & get(params@excused.col0) != 1, denominator := predict(denominator0, newdata = .SD, type = "response")][tx_lag == 0 & get(params@treatment) == 0 & get(params@excused.col0) != 1, denominator := 1 - denominator][tx_lag == 1 & get(params@excused.col1) != 1, denominator := predict(denominator1, newdata = .SD, type = "response")][tx_lag == 1 & get(params@treatment) == 0 & get(params@excused.col1) != 1, denominator := 1 - denominator]

    if (!params@pre.expansion) {
      out <- out[get(params@treatment) == 1 & get(params@excused.col0) == 0, numerator := predict(numerator0, newdata = .SD, type = "response")][get(params@treatment) == 1 & get(params@excused.col1) == 0, numerator := predict(numerator1, newdata = .SD, type = "response")][get(params@treatment) == 0, numerator := 1 - numerator]
    } else {
      out <- out[, numerator := 1]
    }
    setnames(out, params@time, "period")
