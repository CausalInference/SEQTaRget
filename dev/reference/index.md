# Package index

## All functions

- [`SEQ_data()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQ_data.md)
  : Function to return the internal data from a SEQuential object

- [`SEQdata.LTFU`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQdata.LTFU.md)
  :

  Simulated lost-to-followup example data for
  [`SEQuential()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQuential.md)

- [`SEQdata`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQdata.md)
  : Simulated observational example data for SEQuential

- [`SEQdata.multitreatment`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQdata.multitreatment.md)
  :

  Simulated multitreatment example data for
  [`SEQuential()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQuential.md)
  multinomial models

- [`SEQestimate()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQestimate.md)
  : Estimate the (very rough) time to run SEQuential analysis on current
  machine

- [`SEQopts()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQopts.md)
  : Parameter Builder for SEQuential Model and Estimates

- [`SEQoutput-class`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQoutput-class.md)
  : An S4 class used to hold the outputs for the SEQuential process

- [`SEQuential()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQuential.md)
  : SEQuential trial emulation

- [`compevent()`](https://causalinference.github.io/SEQTaRget/dev/reference/compevent.md)
  : Function to return competing event models from a SEQuential object

- [`covariates()`](https://causalinference.github.io/SEQTaRget/dev/reference/covariates.md)
  : Retrieves Outcome, Numerator, and Denominator Covariates

- [`denominator()`](https://causalinference.github.io/SEQTaRget/dev/reference/denominator.md)
  : Retrieves Denominator Models from SEQuential object

- [`diagnostics()`](https://causalinference.github.io/SEQTaRget/dev/reference/diagnostics.md)
  : Function to return diagnostic tables from a SEQuential object

- [`hazard_ratio()`](https://causalinference.github.io/SEQTaRget/dev/reference/hazard_ratio.md)
  : Function to return hazard ratios from a SEQuential object

- [`km_curve()`](https://causalinference.github.io/SEQTaRget/dev/reference/km_curve.md)
  : Function to print Kaplan-Meier curves

- [`km_data()`](https://causalinference.github.io/SEQTaRget/dev/reference/km_data.md)
  : Function to return survival data from a SEQuential object

- [`numerator()`](https://causalinference.github.io/SEQTaRget/dev/reference/numerator.md)
  : Retrieves Numerator Models from SEQuential object

- [`outcome()`](https://causalinference.github.io/SEQTaRget/dev/reference/outcome.md)
  : Retrieves Outcome Models from SEQuential object

- [`risk_comparison()`](https://causalinference.github.io/SEQTaRget/dev/reference/risk_comparison.md)
  : Function to return risk information from a SEQuential object

- [`risk_data()`](https://causalinference.github.io/SEQTaRget/dev/reference/risk_data.md)
  : Function to return risk information from a SEQuential object

- [`show(`*`<SEQoutput>`*`)`](https://causalinference.github.io/SEQTaRget/dev/reference/show-SEQoutput-method.md)
  : Show method for S4 object - SEQoutput.
