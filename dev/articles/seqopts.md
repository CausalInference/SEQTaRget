...
provides the primary API to changing internal options of
[`SEQuential()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQuential.md).
While the documentation should go over their use, we will expand on them
...
| `risk.times` | Follow-up times (in follow-up units) at which to report risk difference and risk ratio when `km.curves = TRUE`; each is snapped to the latest follow-up at or before it and the final time is always included (`NA` reports only the final time) | Numeric vector | `c(36, 120)` |
...
All of these can be changed after
[`SEQuential()`](https://causalinference.github.io/SEQTaRget/dev/reference/SEQuential.md)
has finished analysis - if you forget to add them, no need to stop and
